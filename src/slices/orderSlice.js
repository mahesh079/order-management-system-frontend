...
export const updateQuantity = createAsyncThunk(
  ...
  async ({ orderId, quantity }, thunkAPI) => {
    try {
      await axios.put(
...
export const getFilteredOrders = createAsyncThunk(
  ...
  async ({ name, filterDate }, thunkAPI) => {
    try {
      const response = await axios.get(
        "http://localhost:5000/get-specific-orders",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
          },
      ...
      return thunkAPI.rejectWithValue(
        error.response?.data?.error || "Failed to fetch filtered orders"
      );
    }
  }
);
  ...
  async (orderData, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        "http://localhost:5000/create-new-order",
        orderData,
        {
          headers: {
